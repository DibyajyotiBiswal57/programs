# Language Guides

This page provides an overview of all language‑specific guides available in the **Programming Solutions Universe**.

---

## 1. Primary Guides

- [Python Guide](Language-Guide-Python.md)
- [Java Guide](Language-Guide-Java.md)
- [C Guide](Language-Guide-C.md)
- [C++ Guide](Language-Guide-Cpp.md)

---

## 2. Other Languages

The repository also supports additional languages, including (but not limited to):

- **Classic:** QBasic, Visual Basic, Fortran, Batch, Lua  
- **Modern:** C#, Go, Haskell, Elixir  
- **Scripting:** Ruby, Perl, PowerShell  
- **Systems:** Assembly

[Other Languages](Language-Guide-Other-Languages.md) - Complete list of all supported languages

[← Back to Home](Home.md) 

Last updated: 2026-03-02 11:05:28 UTC
